...
async function createMappingIfNotPresentAsync (options) {
  ...
  let mapping = generator.generateMappingAsync(schema)
  ...
  try {
    ...
    mappingEs = await client.indices.putMapping({
      index: indexName,
      type: typeName,
...
function createMappingIfNotPresent (options) {
  const client = options.client
  const indexName = options.indexName
  const typeName = options.typeName
  const schema = options.schema
...
function Mongoosastic (schema, pluginOpts) {
  ...
  function postSaveAlreadyPopulated (doc) {
   ...
   * @param cb Function
   */
  schema.statics.createMappingAsync = async function createMappingAsync (inSettings) {
    ...
    return await createMappingIfNotPresentAsync({
      client: esClient,
      indexName: indexName,
      typeName: typeName,
      schema: schema,
  ...
  schema.statics.synchronize = function synchronize (inQuery, inOpts) {
    ...
    stream = this.find(query, {}, { timeout: true }).cursor({batchSize : bulk.batch})
   ...
   * @param query - query for documents you want to synchronize
   */
  schema.statics.synchronizeAsync = async function synchronizeAsync (inQuery, inOpts) {
    let em = new events.EventEmitter()
    let closeValues = []
    let counter = 0
    let query = inQuery || {}
      ...
      while(true) {
          ...
          try {
            let cursor = await this
              ...
              .sort("_id")
              .skip(nb_processed)
              .limit(bulk.batch).exec();

              if(!cursor || cursor.length == 0){
                break; // Done processing all, exit outer loop
              }

              nb_processed += cursor.length
